	...
	<script type="text/javascript">
	 var grid;
		$(function(){
			...
			parentField : '_parentId',
				...
				columns : [ [ {
					...
					formatter : function(value, row) {
						var str = '';
						str +='<img calss="iconImg ext-icon-note" title="查看" onclick="showFun(\''+row.id+'\')"> ';
		...
		function deleteResource(id){
			alert(id);
			$.ajax({
				url:sys.contextPath+'/resource/deleteResource.do',
			    type:'post',
				data:{"id":id},
				datatype:'json',
			    success:function(data){
		...
		function showFun(id){
			...
			addDialog = $('#addResrouceDialog').dialog({    
			    ...
			    closed: false,    
			    cache: false,    
			    href: sys.contextPath+'/resource/showResource.do?id='+id,    
			    modal: true,
			    buttons:[{
					text:'修改',
					handler:function(){
						$.ajax({
							url:sys.contextPath+'/resource/updateResource.do',
							type:'post',
							data:$("#addResource").serialize(),
							dataType:'json',
							success:function(data){
